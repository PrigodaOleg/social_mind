...
class Repository {
  ...
  Future<int> _saveItemsConsistently(Map<String, dynamic> items) async {
    ...
    Map<Model, Map<String, dynamic>> paretnItems = _clusterItemsByParent(items);

    // Для каждого домена создаем запись в реестре событий.
    paretnItems = _generateTransactions(paretnItems);
    ...
    try {
      ...
      _rollbackLastTransaction(oldItems);
    }
    return 0;
  }
  ...
  Map<Model, Map<String, dynamic>> _generateTransactions(Map<Model, Map<String, dynamic>> paretnItems) {
  ...
  void _rollbackLastTransaction(Map<String, dynamic> oldItems) {
