...
import 'package:closers/state/task_list/task_list.dart';
...
class TaskListPage extends StatelessWidget {
  ...
  const TaskListPage(this.repository, {super.key, this.id});
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => TaskListBloc(
          repository: repository, parentId: id ?? repository.myId ?? ''),
              ...
              for (final (index, task) in state.tasks.values.indexed)
                TaskListTile(
                  ...
                  backgroundColor: index.isEven
                      ...
                      : Color.lerp(
                          t.colorScheme.surface, t.colorScheme.primary, 0.07),
                  onValueChanged: (bool? isComplited) {
                    b.add(TaskCompletionRequested(
                        task: task, isComplited: isComplited ?? false));
                  },
                  onTitleSubmitted: (submittedText) {
                    b.add(TaskSubmitionRequested(
                        task: task, submittedText: submittedText));
                  },
                  onTitleChanged: (changedText) {
                    context.read<TaskListBloc>().add(TaskChangingRequested(
                        task: task, changedText: changedText));
                  },
                )
            ]),
